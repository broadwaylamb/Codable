...
#if swift(>=3.2)
#else
public typealias JSONEncoder = BackportJSONEncoder
public typealias JSONDecoder = BackportJSONDecoder
#endif
...
open class BackportJSONEncoder {
    // MARK: Options
...
internal class _JSONEncoder : Encoder {
    ...
    internal let options: BackportJSONEncoder._Options
    ...
    fileprivate init(options: BackportJSONEncoder._Options, codingPath: [CodingKey] = []) {
...
fileprivate struct _JSONKeyedEncodingContainer<K : CodingKey> : KeyedEncodingContainerProtocol {
    ...
    private func _converted(_ key: CodingKey) -> CodingKey {
        ...
        case .useDefaultKeys:
            return key
        case .convertToSnakeCase:
            let newKeyString = BackportJSONEncoder.KeyEncodingStrategy._convertToSnakeCase(key.stringValue)
...
open class BackportJSONDecoder {
    // MARK: Options
    ...
    public enum KeyDecodingStrategy {
        ...
        fileprivate static func _convertFromSnakeCase(_ stringKey: String) -> String {
...
#else
            // Find the first non-underscore character
            guard let firstNonUnderscore = stringKey.characters.index(where: { $0 != "_" }) else {
                // Reached the end without finding an _
                return stringKey
            }

            // Find the last non-underscore character
            var lastNonUnderscore = stringKey.characters.index(before: stringKey.endIndex)
            while lastNonUnderscore > firstNonUnderscore && stringKey.characters[lastNonUnderscore] == "_" {
                stringKey.characters.formIndex(before: &lastNonUnderscore)
            }
            ...
            if components.count == 1 {
                // No underscores in key, leave the word as is - maybe already camel cased
                joinedString = String(stringKey.characters[keyRange])
            } else {
                joinedString = ([components[0].lowercased()] + components.suffix(from: 1).map { $0.capitalized }).joined()
            }
            ...
            if (leadingUnderscoreRange.isEmpty && trailingUnderscoreRange.isEmpty) {
                ...
                // Both leading and trailing underscores
                result = String(stringKey.characters[leadingUnderscoreRange]) + joinedString + String(stringKey.characters[trailingUnderscoreRange])
            } else if (!leadingUnderscoreRange.isEmpty) {
                // Just leading
                result = String(stringKey.characters[leadingUnderscoreRange]) + joinedString
            } else {
                // Just trailing
                result = joinedString + String(stringKey.characters[trailingUnderscoreRange])
            }
            return result
#endif
        }
...
internal class _JSONDecoder : Decoder {
    ...
    internal let options: BackportJSONDecoder._Options
    ...
    fileprivate init(referencing container: Any, at codingPath: [CodingKey] = [], options: BackportJSONDecoder._Options) {
...
fileprivate struct _JSONKeyedDecodingContainer<K : CodingKey> : KeyedDecodingContainerProtocol {
    ...
    private func _errorDescription(of key: CodingKey) -> String {
        ...
        case .convertFromSnakeCase:
            ...
            let converted = BackportJSONEncoder.KeyEncodingStrategy._convertToSnakeCase(original)
...
extension _JSONDecoder {
    ...
    fileprivate func unbox(_ value: Any, as type: Decimal.Type) throws -> Decimal? {
        ...
        if let decimal = value as? Decimal {
            return decimal
        } else if let number = value as? NSNumber, number !== kCFBooleanTrue, number !== kCFBooleanFalse {
